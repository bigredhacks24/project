...
async function getFreePeriods(calendar: any, calendars: any[]) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const dayFreePeriods = getFreePeriodsBetweenBusy(
      startTime,
      endTime,
      busyPeriods
    );
...
function getFreePeriodsBetweenBusy(
  start: Date,
  end: Date,
  busyPeriods: any[]
): TimeBlock[] {
  const freePeriods: TimeBlock[] = [];
  ...
  if (currentTime < end) {
    freePeriods.push({ start: new Date(currentTime), end: new Date(end) });
  }
