...
export default function FileUploadAndGallery({
  event,
}: FileUploadAndGalleryProps) {
  ...
  const handleFileUpload = async (
    ...
    try {
      const response = await fetch(
        `/api/files?eventId=${event.event_id}&thumbnail=${isThumbnail}`,
        {
          method: "POST",
          body: formData,
        }
    ...
    <div className="p-4 grid grid-cols-2 w-full">
      <div className="border-r border-gray-300 pr-5">
        <div className="mb-6">
          <p className="text-gray-400 text-sm font-medium">
            {formatDateRange(event.start_timestamp, event.end_timestamp)}
          </p>
            ...
            {event.event_person_attendance.map((attendance, index) => (
              <div key={index} className="flex items-center space-x-2">
                <div className="w-8 h-8 bg-gray-300 rounded-full">
                  <img
                    src={
                      attendance.person.profile_picture ||
        ...
        <div className="grid grid-cols-3 gap-4 items-center">
          <div className="col-span-2">
            ...
            <p className="text-gray-400 mb-2 text-sm">
              Add a thumbnail to {event.name}'s page!
            </p>
            <Input
              id="thumbnailInput"
              type="file"
      ...
      <div className="pl-5">
        <div>
          ...
          <p className="text-gray-400 mb-2 -mt-1">
            Add pics from <span className="font-semibold">{event.name}</span>!
          </p>
          <Input
            id="galleryInput"
            type="file"
